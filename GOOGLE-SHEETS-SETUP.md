...
Every new RSVP is added as a row in the first sheet tab. If the same email address is submitted again, that existing row is updated instead of creating a duplicate. The columns include name, email, attendance, attendance type (full day, ceremony only, or dinner only), guest count, dietary notes, other guest names, other notes, and submission time.
